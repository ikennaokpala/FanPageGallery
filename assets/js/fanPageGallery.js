...
(function($) {
    ...
    var FBGallery = {
        ...
        getCoverPhoto : function (album_id, album_name){
            var photo;
            var pselector = "#album-cover-"+album_id;
            $.getJSON("http://graph.facebook.com/"+album_id, function(album_json) {
                $.getJSON("http://graph.facebook.com/"+album_json.cover_photo, function(photo_json) {
                    if(!(typeof photo_json.source === "undefined")) {
                        photo ="<a href='?id="+album_id+"&albumname="+album_name+"' class='album-cover-image-"+album_id+"' onmouseover='return overlib(\&quot;Click here to view more pictures from the "+album_name+" Album.\&quot;);' onmouseout='nd();' ><img src="+ photo_json.source+" width='180' alt='Fan Page Gallery for "+album_name+" Album' height='180'  border='1' /></a><br/>";
            ...
            $.ajax({
                ...
                success: function(json, textStatus, jpXHR) {
                    $.each(json.data, function(i, album){
                        if(!(album.name == "Cover Photos" || album.name == "Profile Pictures" || album.name == "Wall Photos")){
                            tag_id = "album-cover-"+album.id;
                            FBGallery.getCoverPhoto(album.id, album.name);
                            if(!(typeof album.count === "undefined")) {
                                html += "<li id="+tag_id+">";
                                html += "<span>" + album.name + "</span><br/> <input class='album_id' type='hidden' value="+album.id+" ></li>";
                            }
                        }
                    });
                    html += "</ul>";
